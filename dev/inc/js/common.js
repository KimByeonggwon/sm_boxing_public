...
!function($) {
	...
	var accordianFunc = function () {
		function init () {
			var firstList = $('.fold-list').first().find('.fold-cont');
		...
		function accordianInit () {
			$('.fold-header > .icon').removeClass('on')
			$('.fold-cont').css('display','none');
			$('.fold-list').removeClass('on')
		}

		function accordianDown (_thisEl) {
			$(_thisEl).next('.fold-cont').slideDown(300); // 펼치기
			$(_thisEl).find('.icon').addClass('on'); // 아이콘 온
			$(_thisEl).parent().addClass('on'); // 리스크 온 클래스 추가
		}

		function accordianUp () {
			$('.fold-header > .icon').removeClass('on')
			$('.fold-cont').slideUp(300);
			// $('.fold-cont').css('display','none');
			$('.fold-list').removeClass('on')
		}
		...
		function accordianUi () {
			$('.fold-header').on('click', function () {
				var _listOn = $(this).parent().hasClass('on'),
					_this = $(this);
	...
	function layer_popup(el){
			...
			$el.find('a.og-close').click(function(){
				$el.fadeOut();
				$('.dimm').fadeOut();
				return false;
